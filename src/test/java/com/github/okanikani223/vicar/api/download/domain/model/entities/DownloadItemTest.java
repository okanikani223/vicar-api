package com.github.okanikani223.vicar.api.download.domain.model.entities;

import com.github.okanikani223.vicar.api.download.domain.model.vo.RequestUrl;
import com.github.okanikani223.vicar.api.download.others.web.errors.InValidInputException;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

class DownloadItemTest {

    @Test
    void validate_NullAsId_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem(null, new RequestUrl("http://localhost:8080/test.txt"), "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"id\" must not be blank", actual.getMessage());
    }

    @Test
    void validate_EmptyAsId_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem("", new RequestUrl("http://localhost:8080/test.txt"), "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"id\" must not be blank", actual.getMessage());
    }

    @Test
    void validate_BlankOnlyAsId_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem("     ", new RequestUrl("http://localhost:8080/test.txt"), "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"id\" must not be blank", actual.getMessage());
    }

    @Test
    void validate_NullAsUrl_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem("test", null, "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"requestUrl\" must not be null", actual.getMessage());
    }

    @Test
    void validate_EmptyAsUrl_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem("test", new RequestUrl(""), "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"requestUrl.url\" must not be blank", actual.getMessage());
    }

    @Test
    void validate_BlankOnlyAsUrl_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem("test", new RequestUrl("     "), "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"requestUrl.url\" must not be blank", actual.getMessage());
    }

    @Test
    void validate_InValidUrl_ThrowsInValidInputException() {
        var downloadItem = new DownloadItem("test", new RequestUrl("test.txt"), "", 0, 0);

        var actual = assertThrows(InValidInputException.class, downloadItem::validate);

        assertEquals("\"requestUrl.url\" must be a valid URL", actual.getMessage());
    }
}